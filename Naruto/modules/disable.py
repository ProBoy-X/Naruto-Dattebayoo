...
from Naruto.modules.helper_funcs.handlers import CMD_STARTERS, CustomCommandHandler
from Naruto.modules.helper_funcs.misc import is_module_loaded
...
if is_module_loaded(FILENAME):
    ...
    def disable_module(bot: Bot, update: Update, args: List[str]):
        chat = update.effective_chat
        if len(args) >= 1:
            disable_module = "Naruto.modules." + args[0].rsplit(".", 1)[0]
    ...
    def enable_module(bot: Bot, update: Update, args: List[str]):
        ...
        if len(args) >= 1:
            enable_module = "Naruto.modules." + args[0].rsplit(".", 1)[0]
